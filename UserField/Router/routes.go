...
func Start() {
	...
	engine := gin.Default()
	engine.Use(mid.Cors())
	engine.POST("/register/captcha", au.RegisterCaptcha)
	...
	engine.POST("/passwords/forget/captcha", au.RetrievePasswordCaptcha)
	engine.POST("/passwords/forget", au.RetrievePassword)
